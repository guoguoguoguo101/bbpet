...
func run() -> int:
	...
	var s: Node = APP_STATE_SCRIPT.new()
	s.load_from(path)
	...
	failed += _check("empty name", bool(s.call("set_name", "   ")) == false)
	failed += _check("trim name", bool(s.call("set_name", "  豆包豆包豆包豆包  ")) and s.state.pet.name == "豆包豆包豆包豆包".substr(0, 12))
	s.set_species("cat")
