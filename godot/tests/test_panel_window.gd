...
func run() -> int:
	...
	failed += _check("wizard photo preview", panel.has_node("Margin/Content/PhotoPreview"))

	panel.call("show_kind", "hub")
	failed += _check("hub size", panel.size == Vector2i(300, 620))
	failed += _check("hub preview", panel.has_node("Margin/Content/Hero/Preview"))
	failed += _check("hub fold", panel.get_node("Margin/Content/Head/Close").text == "收起")
	...
	failed += _check(
		...
		< panel.get_node("Margin/Content/Foot").get_index()
	)
	failed += _check("hub settings", panel.get_node("Margin/Content/Foot/Settings").text == "设置")
	...
	failed += _check("hub fold copy", panel_source.contains("收起"))
	failed += _check("photo preview copy", panel_source.contains("PhotoPreview"))
	failed += _check("chat persist", panel_source.contains("set_chat_history"))
	...
	failed += _check("settings photo preview", panel.has_node("Margin/Content/PhotoPreview"))
	...
	failed += _check(
		"settings host room",
		panel.get_node("Margin/Content/HostRoom").text == "我来当校长（本机开房，同事填下面的内网地址）"
	)
	failed += _check("settings host room off", panel.get_node("Margin/Content/HostRoom").button_pressed == false)
