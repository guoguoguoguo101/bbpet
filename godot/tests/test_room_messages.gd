...
func run() -> int:
	...
	var game: Dictionary = RoomMessages.parse_server('{"type":"gameState","game":{"id":"g1"}}')
	failed += _check("parse game", game.ignored == false and game.type == "gameState")
	...
	var dress: Dictionary = RoomMessages.parse_server('{"type":"dress","clientId":"a","dress":{"gear":"raincoat","fx":"rain"}}')
	failed += _check("parse dress", dress.ignored == false and dress.type == "dress")
