...
先另开一个终端跑 `npm run room`。Godot 里左键枢纽 → 去上学；教室可写黑板、点同学加好友。枢纽「好友」会连校长，但不自动打开学校窗。关掉学校窗后连接还在，人还在学校里，再点去上学会回到刚才的地图。设置里学校地址默认 `ws://127.0.0.1:18765`。和 Electron 同学同班时，黑板和加好友应互通。
